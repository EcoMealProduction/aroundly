...
import static com.backend.in.mapper.MapperFixtures.domainIncidentForController;
...
public class IncidentControllerTest {
  ...
  void createIncident() throws Exception {
    when(requestMapper.toDomain(any(IncidentRequestDto.class))).thenReturn(domainIncidentForController);
    when(incidentUseCase.create(domainIncidentForController)).thenReturn(domainIncidentForController);

    ResponseEntity<Incident> response = controller.create(incidentRequestDto);
  ...
  void testFindAllInGivenRange() throws Exception {
    ...
    when(incidentUseCase.findAllInGivenRange(eq(lat), eq(lon), eq(radius)))
        .thenReturn(List.of(domainIncidentForController));
    when(previewResponseMapper.toDto(any(Incident.class))).thenReturn(incidentPreviewResponseDto);
